...
function viewLearn() {
  ...
  const seriesNote = {
    ...
    日常沟通: '具体一句话该怎么说、什么时候不该说，每一课都能落到下一次真实对话里。',
    内容判读: '怎么判断你正在读的那份建议值不值得信。这一个系列不是又一批内容，是一把筛子——看完之后，挑内容的眼光就是你自己的了。',
    责任与相处: '责任不是一种感觉，是一串具体动作；以及怎么跟家人、朋友把关系维持住。这一个系列是他自己点名的三件事。',
    五维认知: '每天只做一道五维题；这里用短课把历史、经济、政治、军事、哲学的分析框架讲懂，题目负责迁移到现实。',
    '书籍与现实': '把书里的概念压成一个现实场景和一个动作，读完马上练，不要求背理论。',
  }[lessonSeries] || '';
...
function todayWhat(p, cfg) {
  ...
  if (p.d.frame < p.tg.frame) parts.push('1 道五维题');
...
function saveContentUpdateUrl() { const u=((document.getElementById('updateUrl')||{}).value||'').trim(); const old=updateSettings(); old.contentUpdateUrl=u || 'https://raw.githubusercontent.com/363280297/cognition-trainer/master/docs/update-manifest.json'; localStorage.setItem('eq-settings-v1',JSON.stringify(old)); renderUpdateStatus('地址已保存'); }
